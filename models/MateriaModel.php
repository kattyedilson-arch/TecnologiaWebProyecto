...
class MateriaModel
{
    ...
    /**
     * Verifica si ya existe una materia con el mismo nombre dentro de una misma
     * carrera (ignora mayúsculas/minúsculas). El mismo nombre en carreras
     * distintas sí se permite.
     * @param string $nombre Nombre a comprobar
     * @param int|null $exceptoId ID a excluir al editar (evita un falso duplicado)
     * @param int|null $idCarrera Carrera donde se busca la coincidencia
     * @return bool True si el nombre ya está en uso
     */
    public function existeNombre($nombre, $exceptoId = null, $idCarrera = null)
    {
        $stmt = $this->pdo->prepare("SELECT COUNT(*) FROM materias WHERE nombre_materia = :nombre AND id_carrera <=> :id_carrera AND id_materia != :excepto");
        $stmt->execute([
            ':nombre'     => trim($nombre),
            ':id_carrera' => $idCarrera,
            ':excepto'    => (int)$exceptoId
        ]);

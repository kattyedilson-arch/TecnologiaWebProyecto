...
//   3. Agrupa por MATERIA ordenando las próximas sesiones por fecha
//      para ver fácilmente la agenda de estudiantes confirmados.
...
// Solo sesiones confirmadas o en proceso próximas (hoy o en el futuro)
$proximas = array_values(array_filter($misTutorias, fn($t) =>
    ($t['estado'] === 'confirmada' || $t['estado'] === 'en_proceso') && $t['fecha'] >= date('Y-m-d')
));

// Orden cronológico global
usort($proximas, fn($a, $b) => ($a['fecha'] . ' ' . $a['hora_inicio']) <=> ($b['fecha'] . ' ' . $b['hora_inicio']));

// Agrupación: materia -> [nombre, sesiones ordenadas cronológicamente]
$porMateria = [];
foreach ($proximas as $t) {
    $porMateria[$t['id_materia']]['nombre'] = $t['nombre_materia'];
    $porMateria[$t['id_materia']]['sesiones'][] = $t;
}
foreach ($porMateria as &$m) {
    usort($m['sesiones'], fn($a, $b) => ($a['fecha'] . ' ' . $a['hora_inicio']) <=> ($b['fecha'] . ' ' . $b['hora_inicio']));
}
unset($m);
  ...
  <?php if ($totalProximas === 0): ?>
    <div class="col-12">
      <div class="alert alert-light border text-center text-muted rounded-3 py-5">
        ...
        Aún no tienes sesiones confirmadas próximas. Cuando aceptes solicitudes de tus estudiantes, aparecerán aquí agrupadas por materia.
      </div>
    </div>
  <?php else: ?>
              ...
              <tbody>
                <?php foreach ($m['sesiones'] as $s): ?>
                  <tr>
                    ...
                    <td>
                      <?php
                        $badgeNivel = 'bg-primary text-white border border-primary-subtle';
                        $iconNivel = 'bi-mortarboard';
                        $textoNivel = 'Pregrado';
                        if (($s['nivel_academico'] ?? 'pregrado') === 'posgrado') {
                          $badgeNivel = 'bg-indigo text-indigo border border-indigo-subtle';
                          $iconNivel = 'bi-award';
                          $textoNivel = 'Posgrado';
                        } elseif (($s['nivel_academico'] ?? 'pregrado') === 'invierno') {
                          $badgeNivel = 'bg-info text-white border border-info-subtle';
                          $iconNivel = 'bi-snow';
                          $textoNivel = 'Invierno';
                        } elseif (($s['nivel_academico'] ?? 'pregrado') === 'verano') {
                          $badgeNivel = 'bg-warning text-dark border border-warning-subtle';
                          $iconNivel = 'bi-sun';
                          $textoNivel = 'Verano';
                        } elseif (!in_array(($s['nivel_academico'] ?? 'pregrado'), ['pregrado', 'posgrado', 'invierno', 'verano'], true)) {
                          $badgeNivel = 'bg-secondary text-white border border-secondary-subtle';
                          $iconNivel = 'bi-book';
                          $textoNivel = htmlspecialchars($s['nivel_academico']);
                        }
                      ?>
                      <span class="badge <?= $badgeNivel ?> rounded-pill px-3 py-1">
